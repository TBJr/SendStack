...
function showApp(sessionData) {
  ...
  els.modePill.innerHTML = `<span></span>${smtp ? "SMTP test mode" : "Sandbox mode"}`;
  if (smtp) els.modePill.style.setProperty("--cyan", "#f3b65a");
  startPolling();
...
async function renderSendingSetup() {
  ...
  els.content.innerHTML = `
    <section class="production-hero">
      <div>
        <span class="readiness-status pending">Live sending locked</span>
        ...
        <p>SendStack is ready for safe local workflow testing. The production path is now explicit, while credentials and incomplete infrastructure cannot be mistaken for an active connection.</p>
      </div>
    ...
    <div class="readiness-layout">
      <section class="panel">
        <div class="panel-head"><div><h2>Launch gates</h2><p>Every item must be verified before live sending is unlocked</p></div><span class="readiness-status locked">Sandbox only</span></div>
